...
Upgrades:
1. Double Metaphone + RapidFuzz Composite Scoring:
   Composite_Score = (0.6 * Token_Sort_Ratio) + (0.4 * Metaphone_Similarity)
2. Dosage-Form & Strength Filtering (Tab, Cap, Syr, Inj, mg, ml).
3. 3-Tier Confidence Status:
   - Score >= 0.88: AUTO_APPROVED / auto_accepted
   - 0.65 <= Score < 0.88: AMBIGUOUS_REQUIRES_CONFIRMATION (top 3 matches)
   - Score < 0.65: MANUAL_REVIEW_REQUIRED
4. Verbal Context Anchoring (Module A -> Module B Context Injection) to re-weight candidates based on clinical indication.
...
def extract_dosage_form_and_strength(raw_text: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    return detected_form, detected_strength
...
def clean_medicine_string(raw_text: str) -> str:
    ...
    # Remove trailing dosage indicators like 500mg, 40mg, 625, etc.
...
def calculate_metaphone_similarity(query: str, candidate: str) -> float:
    ...
    # 1. Primary keys match exactly
    if q_p == c_p:
        return 1.0
    # 2. Primary matches candidate secondary or vice versa
    if (q_s and q_s == c_p) or (c_s and q_p == c_s):
        return 0.85
    # 3. Secondary keys match
    if q_s and c_s and q_s == c_s:
        return 0.75
    # 4. Partial phonetic similarity using Levenshtein ratio on primary metaphones
